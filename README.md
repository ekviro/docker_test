...
```
Запустить тесты локально (в вирт. окруж.)
```
pytest tests/ -v
```
или как обычно в IDE значками выборочно
...
```
В requirements.txt ВСЕ зависимости для сервера и тестов, чтобы удобно ставить в одну виртуальную среду.    
В отдельных файлах - только нужные каждому контейнеру в докере, их локально не ставить
